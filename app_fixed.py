...
PAIRS = {
    ...
    "EUR/ILS": "EURILS=X",   # ILS per 1 EUR  <-- NEW
}
...
def transform_series(s: pd.Series, mode: str):
    ...
    - 'Normalized to 100' -> s / s0 * 100
    - 'Percent change (%)' -> (s / s0 - 1) * 100
    """
...
def make_chart(df: pd.DataFrame, title: str, ma_periods, period: str, norm_mode: str):
    ...
    # MAs computed on raw, then transformed to same baseline
    ma_series = []
    for p in ma_periods:
        try:
            p = int(p)
    ...
    for p, m in ma_series:
        fig.add_trace(go.Scatter(x=m.index, y=m, name=f"MA{p}", line=dict(dash="dot")), row=1, col=1)

    # Volume
    if "Volume" in df.columns and df["Volume"].notna().any():
        fig.add_trace(go.Bar(x=df.index, y=df["Volume"].fillna(0), name="Volume", opacity=0.5), row=2, col=1)
...
def fx_pl_inverse_quote(close_series: pd.Series, amount_local: float):
    """
    For pairs quoted as LOCAL per EUR (EUR/USD, EUR/CNY, EUR/AUD, EUR/GBP, EUR/JPY, EUR/SEK, EUR/ILS):
    EUR value = LOCAL_amount / price.
    """
...
# FX inputs (only for selected pairs)
usd_amount = cny_amount = aud_amount = gbp_amount = jpy_amount = sek_amount = ils_amount = None
...
if "EUR/ILS" in selected:
    ils_amount = st.sidebar.number_input("ILS amount (EUR/ILS)", min_value=500.0, value=3376.0, step=100.0)
...
for name, df in data_map.items():
    with st.expander(f"{name} Chart", expanded=True):
        ...
        if name == "EUR/ILS" and ils_amount:
            res = fx_pl_inverse_quote(df["Close"], ils_amount)
            if res:
                eur_df, stats = res
                st.markdown("**💶 ILS asset → EUR (FX-only)**")
                c1, c2, c3 = st.columns(3)
                c1.metric("Start (EUR)", fmt_num(stats["start_eur"], 2))
                c2.metric("End (EUR)", fmt_num(stats["end_eur"], 2))
                sign = "+" if stats["abs_pl_eur"] >= 0 else ""
                ...
                fx_fig.add_trace(go.Scatter(
                    x=eur_df.index, y=eur_df["EUR_Value"], name=f"EUR value of ₪{int(ils_amount)} (ILS)"
                ))
                fx_fig.update_layout(template="plotly_dark", height=260, margin=dict(l=20, r=20, t=20, b=20))
                fx_fig.update_yaxes(title_text="EUR")
                st.plotly_chart(fx_fig, use_container_width=True)
                st.caption("FX effect only — excludes any change in the underlying Israeli asset price.")
